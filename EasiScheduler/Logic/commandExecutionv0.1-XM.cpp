...
string CommandExecution::readCommand(string userInput) {
    ...
    _content=userInput.substr(end+1);
...
void CommandExecution::storeInTaskInfo() {
    ...
    taskInfo.month=inter.getIntMonth();
...
void CommandExecution::addEventToList() {
    ...
    tasks.addTask(taskInfo.description,  taskInfo.startTime, taskInfo.endTime,taskInfo.day, taskInfo.month, 2015);
}
...
void CommandExecution::performUpdate(string& message) {
    ...
    inter.convert(_content);
    storeInTaskInfo();
    tasks.updateTask(index, taskInfo.description,  taskInfo.startTime, taskInfo.endTime,taskInfo.day, taskInfo.month, 2015);
    ostringstream out;
    out<<MESSAGE_UPDATED;
    message=out.str();
}
