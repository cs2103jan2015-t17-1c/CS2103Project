...
bool Storage::isLocationAvailable(string fileStoringLocation){
	if (std::ifstream(fileStoringLocation)){
		return true;
	}
	else {
		return false;
...
bool Storage::isWithinRange(int index, int maximumSize){
...
bool Storage::isUpdatedTask(int index, string eventdescription, int starttime, int startday, int startmonth,int startyear, int endtime, int endday, int endmonth, int endyear){
	...
	unsigned int i=0;
	while(!isEqualsIndex(_taskLists[i].getUnit(),index) && isWithinRange( i, _taskLists.size())){
	i++;
	}
...
string Storage::displayTasks(){
	...
	for (unsigned i=0; isWithinRange(i,getListSize());i++){
		...
		if (isMarkEqualsZero(i)){
		   ...
		   if(isNormalTask(_taskLists[i].getStartYear(),_taskLists[i].getEndYear())){
			   displayNormalTask(out,_taskLists[i]);
		   } else 
...
bool Storage::isNormalTask(int startYear, int endYear){
...
void Storage::trackNormalTask(ostringstream &outMessage, int unitIndex){
...
string Storage::readTask(int index){
    ostringstream out;
	if (isWithinRange(index+1, getSize())&&isMarkEqualsZero(index-1)){
		out <<"TODO: " <<  _taskLists[index-1].getEventInformation() << "\r\n";
	}else {
		out<<"";
	}
	return out.str();
...
string Storage::searchTask(string content){
	...
	for(iter =  _taskLists.begin();iter !=  _taskLists.end();iter++){
        ...
        if(line.find(content) != string::npos){
			if(isNormalTask((*iter).getStartYear(),(*iter).getEndYear())){			
				displayNormalTask(out,(*iter));
		   }else 
			   if(isDeadlineTask((*iter).getStartYear(),(*iter).getEndYear())){
				displayDeadlineTask(out,(*iter));
			}else{
			   displayFloatingTask(out,(*iter));
		   }
		   searchPosition++;
        }
    }    
...
void Storage::displayDeadlineTask(ostringstream &outMessage,Task &task){
	...
	if(isOverDue(task)){					
		outMessage<<"[Overdue] "<<" Event: " << task.getEventInformation() << "\r\n";				
	}else{					
...
void Storage::displayNormalTask(ostringstream &outMessage,Task &task){
...
bool Storage::operateMark(int index){
	unsigned int i;
...
bool Storage::operateUnmark(int index){
	unsigned int i;
	 for( i=0;isWithinRange(i, _taskLists.size());i++)
    {
        if(isEqualsIndex(index, _taskLists[i].getUnit())){
...
bool Storage::isSortNeeded(int endTime1,int endTime2){
	if(endTime1 == -1 || ( endTime1 >endTime2) && endTime2==-1){
		return true;
	}else{
		return false;
	}
