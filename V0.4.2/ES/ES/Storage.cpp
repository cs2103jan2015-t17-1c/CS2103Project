...
void Storage::updateTask(int index, string eventdescription, int starttime, int startday, int startmonth,int startyear, int endtime, int endday, int endmonth, int endyear){
    ...
    taskLists[i].setEventInformation(eventdescription);
    taskLists[i].setStartTime(starttime);
    taskLists[i].setStartDay(startday);
    taskLists[i].setStartMonth(startmonth);
    taskLists[i].setStartYear(startyear);
	taskLists[i].setEndTime(endtime);
	taskLists[i].setEndDay(endday);
	taskLists[i].setEndMonth(endmonth);
	taskLists[i].setEndYear(endyear);
}
...
string Storage::displayTasks(){
    ...
    for (unsigned i=0; i< taskLists.size();i++){
		if (taskLists[i].getMark()==0){
		   if((taskLists[i].getStartYear()!=-1)&&(taskLists[i].getEndYear()!=-1)){
           out<< setw(6)<<taskLists[i].getUnit()<<" "<< taskLists[i].getStartYear() << "/" <<setw(2)<< taskLists[i].getStartMonth() << "/" <<setw(2)<< taskLists[i].getStartDay() << "/" << taskLists[i].getStartTime() <<" - " << taskLists[i].getEndYear() << "/" <<setw(2)<< taskLists[i].getEndMonth()<< "/" <<setw(2)<< taskLists[i].getEndDay() << "/" << taskLists[i].getEndTime() <<"  "<<" status: "<<"undone"<<" Event: " << taskLists[i].getEventInformation() << endl;
		   }
		   else if((taskLists[i].getStartYear()==-1)&&(taskLists[i].getEndYear()!=-1)){
		   out<< setw(6)<<taskLists[i].getUnit()<<" ----/--/--/---- - " << taskLists[i].getEndYear() << "/" <<setw(2)<< taskLists[i].getEndMonth()<< "/" <<setw(2)<< taskLists[i].getEndDay() << "/" << taskLists[i].getEndTime() <<"  "<<" status: "<<"undone"<<" Event: " << taskLists[i].getEventInformation() << endl;
		   }
		   else{
		   out<< setw(6)<<taskLists[i].getUnit()<<" ----/--/--/---- - ----/--/--/----  " <<" status: "<<"undone"<<" Event: " << taskLists[i].getEventInformation() << endl;
		   }
